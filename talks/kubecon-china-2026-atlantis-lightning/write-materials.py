...
for i, slide in enumerate(slides, 1):
    ...
    if slide['transition']:
        script += [f'Transition: {slide["transition"]}', '']
    timing += [f'| {i} | {slide["seconds"]}s | {stamp(elapsed)}–{stamp(end)} | {words} | {words * 60 / slide["seconds"]:.1f} wpm |']
    cues += [f'{i}. **{stamp(elapsed)} — {slide["title"]}** {slide["cue"]}', f'   Transition: “{slide["transition"]}”' if slide['transition'] else '   Final line: “From the pull request.”', '']
    outline += [f'## {i}. {slide["title"]}', '', slide['purpose'], '', f'Target: {stamp(elapsed)}–{stamp(end)}.', '']
    elapsed = end
script += ['## Emergency 4-minute version', '', 'Use the same five slides. Skip exactly these sentences; do not speak faster. Keep the definition, full-plan review, configured approvals, dated survey evidence, and final line.', '']
for i, slide in enumerate(slides, 1):
    for sentence in slide['emergency_skip']:
